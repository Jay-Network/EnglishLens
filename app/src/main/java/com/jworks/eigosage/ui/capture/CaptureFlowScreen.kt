...
fun CaptureFlowScreen(
    ...
    onRewardsClick: () -> Unit = {},
    onStudyClick: () -> Unit = {},
    resumeChatSessionId: String? = null,
    viewModel: CaptureFlowViewModel = hiltViewModel()
) {
    // Resume a chat session if navigated from history
    LaunchedEffect(resumeChatSessionId) {
        if (resumeChatSessionId != null) {
            viewModel.resumeChatSession(resumeChatSessionId)
        }
    }
